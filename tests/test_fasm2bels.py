...
class TestFasm2Bels(unittest.TestCase):
    ...
    def test_simple_ff(self, test_name):
        ...
        bitread = os.path.join(base_dir, 'third_party', 'prjxray', 'build', 'tools', 'bitread')
        bit_file = os.path.join(cur_dir, 'test_data', '{}.bit'.format(test_name))
        pcf = os.path.join(cur_dir, 'test_data', '{}.pcf'.format(test_name))
        eblif = os.path.join(cur_dir, 'test_data', '{}.eblif'.format(test_name))

        temp_dir = tempfile.mkdtemp(prefix="test_fasm2bels_{}_".format(test_name), dir='/tmp')
        ...
        sys.argv = [
            ...
            '--drive', drive,
            '--connection_database', channels_file,
            generated_top_v, generated_top_tcl
        ]

        main()

        self.assertTrue(os.path.exists(os.path.join(temp_dir, generated_top_v)))
        self.assertTrue(os.path.exists(os.path.join(temp_dir, generated_top_tcl)))
